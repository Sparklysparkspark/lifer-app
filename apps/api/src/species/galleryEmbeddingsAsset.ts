...
const APPLIED_KEY = "gallery_embeddings_version";
...
async function doUpdate(
  ...
  try {
    gallery = await doGalleryUpdate(pool, ctx, manifest, force);
  } catch (err) {
...
async function doGalleryUpdate(pool: Pool, ctx: Ctx, manifest: CatalogManifest, force: boolean): Promise<GalleryEmbeddingsResult> {
  const asset = manifest.galleryEmbeddings;
  if (!asset) return { status: "unavailable", reason: "This catalog has no separate gallery embeddings" };
  if (asset.modelVersion !== EMBEDDING_MODEL_VERSION) {
    return {
      status: "unavailable",
      reason: `Published vectors are for model ${asset.modelVersion}, this install uses ${EMBEDDING_MODEL_VERSION}`,
    };
  }
  const appliedTag = `${manifest.version}:${asset.modelVersion}`;
  if (!force && (await getInstallSetting<string>(pool, APPLIED_KEY)) === appliedTag) return { status: "up_to_date" };
  ...
  const result = await applyGalleryEmbeddingsFile(pool, dest, appliedTag, ctx);
...
export async function applyGalleryEmbeddingsFile(
  ...
  try {
    ...
    let rows = 0;
    ctx.update({ phase: "applying_gallery_embeddings", downloadedBytes: null, totalBytes: null, processed: 0, total: null });
    ...
    const modelVersion = (header as GalleryEmbeddingsHeader | null)?.modelVersion ?? EMBEDDING_MODEL_VERSION;
    ...
    if (appliedTag) await setInstallSetting(client, APPLIED_KEY, appliedTag);
